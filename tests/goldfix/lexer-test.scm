...
(let ((lexer (make-lexer "123 456")))
  ...
  (let ((whitespace-token (lexer-next-token lexer)))
    (check (whitespace-token? whitespace-token) => #t)
    (check (token-lexeme whitespace-token) => " "))
...
(let ((lexer (make-lexer "#b101 #xff 789")))
  ...
  (let ((whitespace1 (lexer-next-token lexer)))
    (check (whitespace-token? whitespace1) => #t)
    (check (token-lexeme whitespace1) => " "))
  (let ((token2 (lexer-next-token lexer)))
    (check (number-token? token2) => #t)
    (check (token-lexeme token2) => "#xff")
    (check (token-value token2) => 255))
  (let ((whitespace2 (lexer-next-token lexer)))
    (check (whitespace-token? whitespace2) => #t)
    (check (token-lexeme whitespace2) => " "))
